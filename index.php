...
'<p class="field"><label for="dc_active">'.__('Active:').'</label> '.
form::checkbox('dc_active',1,$dc_active).'</p>'.
'</fieldset>';

echo
'<fieldset><legend>'.__('Category').'</legend>';

$rs = $core->blog->getCategories(array('post_type'=>'post'));
if ($rs->isEmpty()) {
	echo '<p>'.__('No category yet.').'</p>';
} else {
	echo '<p class="field"><label for="dc_category" class="classic">'.__('Select category:').'</label>'.
